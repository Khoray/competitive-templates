...
int main() {
    ...
    int n = read();
    Poly a(n + 1);
    vector<int> target(n + 1), targety(n + 1), ans(n + 1);
    vector<Poly> divd(4 * n);
    // auto divide_mul2 = [&](auto divide_mul2, int l, int r) -> Poly {
    //     if(l == r) {
    //         return Poly({mod - target[l], 1});
    //     }
    //     int mid = l + r >> 1;
    //     return divide_mul2(divide_mul2, l, mid, id << 1) * divide_mul2(divide_mul2, mid + 1, r, id << 1 | 1);
    //     // divd[id] = divd[id << 1 | 1] * divd[id << 1];
    // };
    auto divide_mul = [&](auto divide_mul, int l, int r, int id) -> void {
        if(l == r) {
            divd[id] = Poly({mod - target[l], 1});
            return;
        }
        int mid = l + r >> 1;
    ...
    auto getans = [&](auto getans, int l, int r, int id, Poly now) -> void {
        ...
        getans(getans, l, mid, id<<1, now % divd[id << 1]);
        getans(getans, mid + 1, r, id<<1|1, now % divd[id << 1 | 1]);
    };
    for(int i = 1; i <= n; i++) {
        target[i] = read();
        targety[i] = read();
