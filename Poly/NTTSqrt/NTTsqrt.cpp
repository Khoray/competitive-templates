...
int tmps[N],inv_ans_sqrt[N];
void Sqrt(int y[], int len, int ans[]) {
    ...
    for(int i=2;i<=len;i<<=1) {
        ...
        memset(inv_ans_sqrt, 0, sizeof(inv_ans_sqrt));
        inv(ans, i, inv_ans_sqrt);
        ntt(tmps, i<<1, 1),ntt(inv_ans_sqrt, i<<1, 1);
        for(int j=0;j<i<<1;j++) {
            tmps[j] = 1ll * tmps[j] * inv_ans_sqrt[j] % mod;
        }
        ntt(tmps, i<<1, -1);
        for(int j=0;j<i;j++) {
            ans[j] = 1ll * (tmps[j] + ans[j]) % mod * qpow(2) % mod;
        }
        for(int j=i;j<len<<1;j++) {
            ans[j]=0;
